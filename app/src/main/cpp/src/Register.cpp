...
void register_inner_class(JNIEnv *env) {
    int ret = find_class(env, "com/huican/test/bean/DataBean$Inner", &m_inner_block.clazz);
    if (ret != 0) {
...
void register_data_bean_class(JNIEnv *env) {
    int ret = find_class(env, "com/huican/test/bean/DataBean", &m_data_bean_block.clazz);
    if (ret != 0) {
    ...
    get_field(env, &clazz, "mInner", "Lcom/huican/test/bean/DataBean$Inner;", &m_data_bean_block.inner);
